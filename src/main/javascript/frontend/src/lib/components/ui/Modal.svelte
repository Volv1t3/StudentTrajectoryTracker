...
{#if open}
  <div class="fixed inset-0 bg-black/50 z-40 flex items-center justify-center p-4" transition:fade={{ duration: 200 }}>
    <div class="rounded-2xl shadow-2xl z-50 w-full {maxWidth[size]}" transition:fly={{ y: 16, duration: 200 }} style="background: var(--bg-surface);">
      <header class="flex items-start justify-between mb-4 p-6 pb-0">
      ...
      {#if footer}
        <div class="flex justify-end gap-3 p-6 pt-4 border-t border-[--border]">
          {@render footer()}
        </div>
      {/if}
    </div>
