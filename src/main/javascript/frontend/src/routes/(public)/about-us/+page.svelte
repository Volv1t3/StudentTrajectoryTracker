...
<section class="py-20 md:py-28" style="background: var(--bg-primary);">
  <div class="max-w-3xl mx-auto px-4 sm:px-12 lg:px-14 text-center">
    ...
    <p class="text-xl md:text-2xl leading-relaxed font-medium" style="color: var(--text-primary);">
      {data.content.identity_text || 'D.Lab es el Laboratorio de Investigación aplicada de la USFQ.'}
    </p>
  </div>
</section>
...
<section class="py-16 md:py-24" style="background: var(--bg-surface);">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="text-center mb-12">
      <span class="text-lg uppercase tracking-widest block mb-4" style="color: var(--accent); font-family: var(--font-subheading);">Nuestros Valores</span>
      <div class="w-12 h-0.5 mx-auto mb-6" style="background: var(--accent); opacity: 0.3;"></div>
    </div>
    <div class="grid grid-cols-1 md:grid-cols-6 lg:grid-cols-12 gap-5">
      {#each data.valuePropositions as value, i (value.id)}
        <article class={`btn-grid border-2 p-10 bg-[--text-primary] hover:bg-[--color-red-hover] text-md font-subheading rounded-lg transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md active:translate-y-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[--color-red] focus-visible:ring-offset-2 ${i % 5 === 0 ? 'md:col-span-6 lg:col-span-7' : i % 5 === 1 ? 'md:col-span-3 lg:col-span-5' : i % 5 === 2 ? 'md:col-span-3 lg:col-span-4' : i % 5 === 3 ? 'md:col-span-6 lg:col-span-8' : 'md:col-span-6 lg:col-span-4'}`}>
            <h4 class="font-semibold text-[--text-primary] text-md">{value.title}</h4>
            <p class="text-xs text-[--text-secondary] mt-1 leading-relaxed">{value.description}</p>
        </article>
      {/each}
    </div>
  </div>
